    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      {todaySchedule.length > 0 && (
        <div className="card border-l-4 border-emerald-500">
          <h2 className="font-bold text-gray-800 text-sm mb-3">📅 Today ({today})</h2>
          <div className="space-y-2">
            {todaySchedule.map(e => (
              <div key={e.id} className="flex items-center gap-4 bg-emerald-50 rounded-lg px-4 py-2.5">
                <div className="w-8 text-center">
                  <p className="font-bold text-emerald-600 text-sm">P{e.period_number}</p>
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-gray-800 text-sm">{e.subject_name || '—'}</p>
                  <p className="text-xs text-gray-500">Class: {e.class_id}</p>
                </div>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100">
          <h2 className="font-bold text-gray-800 text-sm">Weekly Teaching Schedule</h2>
        </div>
              ...
              <tr>
                ...
                {DAYS.map(d => (
                  <th key={d}
                    ...
                    {d.slice(0, 3)}
                    {d === today && <span className="ml-1 text-emerald-200">●</span>}
                  </th>
                ))}
              </tr>
            </thead>
                  ...
                  {DAYS.map(day => {
                    ...
                    return (
                      <td key={day}
                        ...
                        {cell ? (
                          <div>
                            <p className={`font-semibold truncate ${isToday ? 'text-emerald-700' : 'text-gray-700'}`}>
                              {cell.subject_name || '—'}
                            </p>
                            {cell.room_number && (
                              <p className="text-gray-400 text-xs">🚪 {cell.room_number}</p>
                            )}
                          </div>
                        ) : (
